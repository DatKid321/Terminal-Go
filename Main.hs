...
stepMoves :: Board -> [(Player, Coord)] -> IO () -- Allow the user to step through a given SGF
stepMoves board ((p, c) : moves) = do
  putStrLn $ "\ESC[H" ++ printBoard board
  putStr "\ESC[0J"
  putStr "Press Enter for next move (or 'q' to quit): "
  hFlush stdout
  line <- getLine
  when (line /= "q") $ maybe (putStrLn $ "Illegal move: " ++ show c) (`stepMoves` moves) (placeStone board p c)
...
main = do
  ...
  let size = 19
      ...
      handle (coords, "") = stepMoves newBoard coords
