...
stepMoves :: Rules -> Position-> [(Player, Point)] -> IO ()
stepMoves rules pos ((player, point) : moves) = do
    putStrLn $ "\ESC[H" ++ printBoard rules pos
    putStr "\ESC[0J"
    putStr "Press Enter for next move (or 'q' to quit): "
    hFlush stdout
    line <- getLine
    when (line /= "q") $ bool (putStrLn $ "Illegal move: " ++ show point) (stepMoves rules (move rules player point pos) moves) (pos point == Empty)

stepMoves rules pos [] = do
    putStrLn $ "\ESC[H" ++ printBoard rules pos
    putStr "\ESC[0J"
